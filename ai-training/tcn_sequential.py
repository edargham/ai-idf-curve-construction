...
from shared_optuna_tuning import (
    create_sequential_tcn_objective,
    run_optuna_study,
    save_optuna_results,
    set_seed,
    get_device,
...
print("\n" + "="*80)
print("CREATING SEQUENTIAL IDF DATASETS FOR TCN")
print("="*80)
...
print('\n' + "="*80)
print('STARTING OPTUNA HYPERPARAMETER TUNING FOR SEQUENTIAL TCN')
print("="*80)

tcn_objective = create_sequential_tcn_objective(
    train_dataset, val_dataset, device, max_epochs=200
)
study = run_optuna_study(tcn_objective, "TCN_Sequential", n_trials=1, direction="maximize")

# Save Optuna results
best_params_df, trials_df = save_optuna_results(study, "TCN_Sequential")
...
print("\n" + "="*80)
print("TRAINING FINAL TCN MODEL WITH BEST HYPERPARAMETERS")
print("="*80)

final_model = SequentialTCN(
    num_channels=13,
    seq_len=best_params['seq_len'],
...
print("\n" + "="*80)
print("FINAL TCN VALIDATION METRICS")
print("="*80)
print(f"  NSE:  {nse:.6f}")
...
print("\nTCN Validation Metrics by Duration:")
...
print("\n" + "="*80)
print("TCN TRAINING AND EVALUATION COMPLETE")
print("="*80)
