...
# Replace SVR with a PyTorch TCN, using MPS when available and 5-fold cross-validation
...
class TCN(nn.Module):
    def __init__(self, seq_len=3, num_filters=144, kernel_size=3, dropout=0.01, num_levels=2):
...
for h in hidden_candidates:
    ...
    for train_idx, val_idx in kf.split(X_train_arr):
        ...
        # For the TCN we treat the feature vector as a short sequence (seq_len = n_features)
        seq_len = X_tr.shape[1]
        model_cv = TCN(seq_len=seq_len, num_filters=h, kernel_size=3, dropout=0.01, num_levels=2).to(device)
        opt = torch.optim.AdamW(model_cv.parameters(), lr=8e-4, weight_decay=1e-6)
...
final_model = TCN(seq_len=X_train_arr.shape[1], num_filters=best_hidden, kernel_size=3, dropout=0.01, num_levels=2).to(
    device
)
opt = torch.optim.AdamW(final_model.parameters(), lr=8e-4, weight_decay=1e-6)
criterion = nn.MSELoss()
    ...
    if loader_val is not None:
        ...
        if not val_df_combined.empty and len(y_pred_intensity) > 0:
            ...
            for dmin, col in zip(duration_minutes, col_names):
                ...
                if len(obs) == 0:
                    tcn_duration_metrics_epoch[col] = {
                        "R2": np.nan,
                        "NSE": np.nan,
                        "MAE": np.nan,
                        "RMSE": np.nan,
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(
                f"Epoch {epoch}/{num_epochs} - train_loss: {tr_loss:.6f} - val_rmse: {rmse:.4f} - val_mae: {mae:.4f} - val_r2: {r2_model:.4f} - val_nse: {nse:.4f}"
            )

    else:
        if epoch % 50 == 0 or epoch == 1:
            print(
                f"Epoch {epoch}/{num_epochs} - train_loss: {tr_loss:.6f} (no year-split validation available)"
            )
...
if valid_lit_metrics:
    ...
    if os.path.exists(lit_metrics_file):
        ...
        # Check if TCN already exists in the dataframe
        if "TCN" in lit_metrics_df["Model"].values:
            # Update existing row
            for col, val in lit_comparison_metrics.items():
                lit_metrics_df.loc[lit_metrics_df["Model"] == "TCN", col] = val
        else:
            # Append new row
