...
def calculate_validation_metrics(val_df, durations, model_params, model_type="gev"):
    ...
    for j, dur in enumerate(durations):
        ...
        if model_type == "gev":
            ...
            predicted = stats.genextreme.ppf(q=qs, c=shape, loc=loc, scale=scale)
...
def fit_gev_distribution(train_df, durations, return_periods, probabilities):
    """
    Fit Generalized Extreme Value (GEV) distribution on training data and calculate intensities.
    ...
    Returns:
        tuple: (gev_params, intensities_gev)
            - gev_params: Dictionary of (shape, loc, scale) for each duration
            - intensities_gev: 2D array of intensities [return_periods x durations]
    """
    gev_params = {}
    intensities_gev = np.zeros((len(return_periods), len(durations)))
    ...
    for j, dur in enumerate(durations):
        ...
        if len(train_values) < 2:
            ...
            gev_params[dur] = (np.nan, np.nan, np.nan)
            intensities_gev[:, j] = np.nan
            continue

        # Fit GEV distribution (shape, loc, scale)
        shape, loc, scale = stats.genextreme.fit(train_values)
        gev_params[dur] = (shape, loc, scale)

        for i, prob in enumerate(probabilities):
            intensities_gev[i, j] = stats.genextreme.ppf(q=prob, c=shape, loc=loc, scale=scale)
    
    return gev_params, intensities_gev
...
def main():
    """Main function to run GEV distribution analysis."""
    
    print("\n" + "=" * 60)
    print("GEV DISTRIBUTION IDF ANALYSIS")
    print("=" * 60)
    ...
    print("\nFitting GEV distribution on training data...")
    gev_params, intensities_gev = fit_gev_distribution(
        train_df, durations, return_periods, probabilities
    )
    
    print("\nGEV Distribution Parameters:")
    for dur in durations:
        shape, loc, scale = gev_params[dur]
        print(f"{dur}: shape = {shape:.4f}, location = {loc:.4f}, scale = {scale:.4f}")
    
    # Save GEV parameters to CSV
    save_distribution_parameters(gev_params, "GEV")
    
    # Validate on validation set
    print("\n" + "=" * 60)
    print("VALIDATION ON 2019-2025 USING GEV FIT FROM 1998-2018")
    print("=" * 60)
    
    duration_metrics = calculate_validation_metrics(
        val_df, durations, gev_params, "gev"
    )
    ...
    try:
        ...
        print(
            f"Warning: failed to save GEV metrics to model_performance_metrics.csv: {e}"
        )
    ...
    idf_data = pd.DataFrame(intensities_gev, index=return_periods, columns=duration_labels)
    idf_data.index.name = "Return Period (years)"
    
    output_dir = os.path.join(os.path.dirname(__file__), "..", "results")
    idf_data.to_csv(os.path.join(output_dir, "idf_data_gev.csv"))
    print(f"IDF data saved to: {os.path.join(output_dir, 'idf_data_gev.csv')}")
    ...
    create_idf_plot(
        ...
        duration_hours,
        colors,
        "Intensity-Duration-Frequency (IDF) Curves - GEV",
        "idf_curves_gev.png",
        avg_metrics,
    ...
    if os.path.exists(lit_data_path):
        ...
        comparison_metrics = compare_with_literature(
            idf_data, lit_data, return_periods, "GEV"
        )
        ...
        plot_model_vs_literature_comparison(
            ...
            "GEV",
            "idf_comparison_gev.png",
            comparison_plot_metrics,
        )
        print(
            f"\nLiterature comparison plot saved to: "
            f"{os.path.join(os.path.dirname(__file__), '..', 'figures', 'idf_comparison_gev.png')}"
        )
        ...
        if os.path.exists(lit_metrics_file):
            ...
            # Check if GEV already exists
            if "GEV" in existing_metrics["Model"].values:
                # Update existing row
                for col, val in comparison_metrics_row.items():
                    existing_metrics.loc[
                        existing_metrics["Model"] == "GEV", col
                    ] = val
            else:
                # Append new row
