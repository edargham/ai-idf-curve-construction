...
def calculate_validation_metrics(val_df, durations, model_params, model_type="gumbel"):
    ...
    for j, dur in enumerate(durations):
        ...
        if model_type == "gumbel":
            loc_scale = model_params.get(dur, (np.nan, np.nan))
            loc, scale = loc_scale
            ...
            predicted = stats.gumbel_r.ppf(q=qs, loc=loc, scale=scale)
...
def fit_gumbel_distribution(train_df, durations, return_periods, probabilities):
    """
    Fit Gumbel distribution on training data and calculate intensities for return periods.
    ...
    Returns:
        tuple: (gumbel_params, intensities_gum)
            - gumbel_params: Dictionary of (loc, scale) for each duration
            - intensities_gum: 2D array of intensities [return_periods x durations]
    """
    gumbel_params = {}
    ...
    for j, dur in enumerate(durations):
        ...
        if len(train_values) < 2:
            ...
            gumbel_params[dur] = (np.nan, np.nan)
            intensities_gum[:, j] = np.nan
            continue

        loc, scale = stats.gumbel_r.fit(train_values)
        gumbel_params[dur] = (loc, scale)

        for i, prob in enumerate(probabilities):
            intensities_gum[i, j] = stats.gumbel_r.ppf(q=prob, loc=loc, scale=scale)
    
    return gumbel_params, intensities_gum
...
def main():
    """Main function to run Gumbel distribution analysis."""
    
    print("\n" + "=" * 60)
    print("GUMBEL DISTRIBUTION IDF ANALYSIS")
    print("=" * 60)
    ...
    print("\nFitting Gumbel distribution on training data...")
    gumbel_params, intensities_gum = fit_gumbel_distribution(
    ...
    print("\n" + "=" * 60)
    print("VALIDATION ON 2019-2025 USING GUMBEL FIT FROM 1998-2018")
    print("=" * 60)
    
    duration_metrics = calculate_validation_metrics(
        val_df, durations, gumbel_params, "gumbel"
    )
    ...
    idf_data = pd.DataFrame(intensities_gum, index=return_periods, columns=duration_labels)
    idf_data.index.name = "Return Period (years)"
    
    output_dir = os.path.join(os.path.dirname(__file__), "..", "results")
    idf_data.to_csv(os.path.join(output_dir, "idf_data.csv"))
    print(f"IDF data saved to: {os.path.join(output_dir, 'idf_data.csv')}")
    ...
    if os.path.exists(lit_data_path):
        ...
        comparison_metrics = compare_with_literature(
            idf_data, lit_data, return_periods, "Traditional Gumbel"
        )
        ...
        plot_model_vs_literature_comparison(
            ...
            "Traditional Gumbel",
            "idf_comparison_trad.png",
            comparison_plot_metrics,
        )
        print(
            f"\nLiterature comparison plot saved to: "
            f"{os.path.join(os.path.dirname(__file__), '..', 'figures', 'idf_comparison_trad.png')}"
        )
        
        # Save comparison metrics
        comparison_metrics_row = {
            "Model": "Traditional_Statistical",
            "RMSE": overall_rmse,
            "MAE": overall_mae,
            "R2": overall_r2,
            "NSE": overall_nse,
        ...
        if os.path.exists(lit_metrics_file):
            ...
            # Check if Traditional_Statistical already exists
            if "Traditional_Statistical" in existing_metrics["Model"].values:
                # Update existing row
                for col, val in comparison_metrics_row.items():
                    existing_metrics.loc[
                        existing_metrics["Model"] == "Traditional_Statistical", col
                    ] = val
            else:
                # Append new row
    ...
    print("\n" + "=" * 60)
    print("GUMBEL ANALYSIS COMPLETE")
    print("=" * 60)
