...
def calculate_validation_metrics(val_df, durations, model_params, model_type="weibull"):
    ...
    for j, dur in enumerate(durations):
        ...
        if model_type == "weibull":
            ...
            predicted = stats.weibull_min.ppf(q=qs, c=shape, loc=loc, scale=scale)
...
def fit_weibull_distribution(train_df, durations, return_periods, probabilities):
    """
    Fit Weibull distribution on training data and calculate intensities for return periods.
    ...
    Returns:
        tuple: (weibull_params, intensities_weibull)
            - weibull_params: Dictionary of (shape, loc, scale) for each duration
            - intensities_weibull: 2D array of intensities [return_periods x durations]
    """
    weibull_params = {}
    intensities_weibull = np.zeros((len(return_periods), len(durations)))
    ...
    for j, dur in enumerate(durations):
        ...
        if len(train_values) < 2:
            ...
            weibull_params[dur] = (np.nan, np.nan, np.nan)
            intensities_weibull[:, j] = np.nan
            continue

        # Fit Weibull distribution (shape, loc, scale)
        shape, loc, scale = stats.weibull_min.fit(train_values, floc=0)
        weibull_params[dur] = (shape, loc, scale)

        for i, prob in enumerate(probabilities):
            intensities_weibull[i, j] = stats.weibull_min.ppf(q=prob, c=shape, loc=loc, scale=scale)
    
    return weibull_params, intensities_weibull
...
def main():
    """Main function to run Weibull distribution analysis."""
    
    print("\n" + "=" * 60)
    print("WEIBULL DISTRIBUTION IDF ANALYSIS")
    print("=" * 60)
    ...
    print("\nFitting Weibull distribution on training data...")
    weibull_params, intensities_weibull = fit_weibull_distribution(
        train_df, durations, return_periods, probabilities
    )
    
    print("\nWeibull Distribution Parameters:")
    for dur in durations:
        shape, loc, scale = weibull_params[dur]
        print(f"{dur}: shape = {shape:.4f}, location = {loc:.4f}, scale = {scale:.4f}")
    
    # Save Weibull parameters to CSV
    save_distribution_parameters(weibull_params, "Weibull")
    
    # Validate on validation set
    print("\n" + "=" * 60)
    print("VALIDATION ON 2019-2025 USING WEIBULL FIT FROM 1998-2018")
    print("=" * 60)
    
    duration_metrics = calculate_validation_metrics(
        val_df, durations, weibull_params, "weibull"
    )
    ...
    try:
        ...
        print(
            f"Warning: failed to save Weibull metrics to model_performance_metrics.csv: {e}"
        )
    
    # Save IDF data
    print("\nSaving IDF curves data...")
    idf_data = pd.DataFrame(intensities_weibull, index=return_periods, columns=duration_labels)
    idf_data.index.name = "Return Period (years)"
    
    output_dir = os.path.join(os.path.dirname(__file__), "..", "results")
    idf_data.to_csv(os.path.join(output_dir, "idf_data_weibull.csv"))
    print(f"IDF data saved to: {os.path.join(output_dir, 'idf_data_weibull.csv')}")
    ...
    create_idf_plot(
        ...
        duration_hours,
        colors,
        "Intensity-Duration-Frequency (IDF) Curves - Weibull",
        "idf_curves_weibull.png",
        avg_metrics,
    ...
    if os.path.exists(lit_data_path):
        ...
        comparison_metrics = compare_with_literature(
            idf_data, lit_data, return_periods, "Weibull"
        )
        ...
        plot_model_vs_literature_comparison(
            ...
            "Weibull",
            "idf_comparison_weibull.png",
            comparison_plot_metrics,
        )
        print(
            f"\nLiterature comparison plot saved to: "
            f"{os.path.join(os.path.dirname(__file__), '..', 'figures', 'idf_comparison_weibull.png')}"
        )
        ...
        if os.path.exists(lit_metrics_file):
            ...
            # Check if Weibull already exists
            if "Weibull" in existing_metrics["Model"].values:
                # Update existing row
                for col, val in comparison_metrics_row.items():
                    existing_metrics.loc[
                        existing_metrics["Model"] == "Weibull", col
                    ] = val
            else:
                # Append new row
